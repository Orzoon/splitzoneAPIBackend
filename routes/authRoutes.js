...
// });
authRoutes.get("/google/callback", passport.authenticate("google", { failureRedirect: "/LS", session: false }),
    function(req, res) {
        //NOTE if token is not present redirect to login page
        const token = req.user.createdToken;
        return res.redirect("http://localhost:3000/social?token=" + token);
    }
);
